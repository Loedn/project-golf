...
Asiago, golf as you have never known!", price: 80.00, owner: user, disabled_days: [])
18.times do |i|
  hole = Hole.new(name: '#{i+1}', par: 3)
  hole.course = course2
  hole.save!
...
course3 = Course.create!(image:"image/upload/v1504274734/shooters.jpg", name: 'G.C. Shooters Hill', address: 'Eaglesfield Rd, London SE18 3DA', description: "Shooter's Hill GC is an attractive parkland course situated close to Blackheath, with superb views across the North Kent countryside. The course is a challenging but fair test of golf, good shot selection is critical for a good score. The course has a number of challenging holes. Our signature hole is the picturesque tenth (shown right) which is included in our own Amen Corner consisting of three par 3's each different to the others and each very demanding. Completing these in par is essential to scoring a good round.", price: 45, owner: user, disabled_days: [])
18.times do |i|
  hole = Hole.new(name: '#{i+1}', par: 3)
  hole.course = course3
  hole.save!
...
course7 = Course.create!(image:"image/upload/v1504274733/london.jpg", name: 'G.C. London', address: 'Stansted Lane, Ash, Nr Brands Hatch, Kent TN15 7EH', description: "A classic Jack Nicklaus course, which forces you to consider every shot. Dramatic tee shots and daunting approaches make this course both memorable and challenging. Host to the 2008 & 2009 European Open, the course is maintained to the very best standards all-year round.", price: 120, owner: user, disabled_days: [])
18.times do |i|
  hole = Hole.new(name: '#{i+1}', par: 3)
  hole.course = course7
  hole.save!
